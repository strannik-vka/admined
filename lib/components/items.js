...
class Items extends React.Component {
  ...
  with(input, item) {
    return typeof input.with !== 'undefined' ? item[input.with] : item[input.name.replace('_id', '')];
  }
  ...
  file(item, input) {
    ...
    return image ? /*#__PURE__*/React.createElement("a", {
      href: item[name],
      target: "_blank"
    }, /*#__PURE__*/React.createElement("img", {
      src: imageUrl(item[name], input.thumb),
      className: "image"
    })) : fileUrls.length ? fileUrls.map((url, i) => {
  ...
  actions(item) {
    ...
    if (isActions) {
      return /*#__PURE__*/React.createElement("td", null, /*#__PURE__*/React.createElement("div", {
        className: "actions"
      }, this.props.page.config('deleteAction', true) ? /*#__PURE__*/React.createElement(Checkbox, {
        checked: this.props.itemsSelected.indexOf(item.id) > -1,
        onChange: () => this.props.itemSelect(item.id)
      }) : '', this.props.page.config('editAction', true) ? /*#__PURE__*/React.createElement(OverlayTrigger, {
        key: "top",
        placement: "top",
  ...
  render() {
    return this.props.paginate.data.length ? this.props.paginate.data.map(item => /*#__PURE__*/React.createElement("tr", {
      key: item.id
    }, this.actions(item), this.props.page.form.map(input => input.filter === false ? false : /*#__PURE__*/React.createElement("td", {
      key: item.id + '_' + input.name,
      className: input.center ? 'text-center' : ''
    }, this[input.type ? input.type : 'string'](item, input))))) : /*#__PURE__*/React.createElement("tr", {
