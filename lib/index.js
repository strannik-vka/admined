...
class Admined extends React.Component {
  ...
  onFilterChange = (value, name, callback) => {
    ...
    this.setState(Object.assign(this.pageDefault, {
      page: pageData
    }), () => {
      if (callback) {
        callback();
  ...
  getItems() {
    axios.get(location.pathname + '/' + this.state.page.url, {
      ...
      this.setState({
        paginate: {
          data: paginate.data ? [...this.state.paginate.data, ...paginate.data] : this.state.paginate.data,
          total: paginate.total
        },
        page: Object.assign(this.pageDefault.page, {
          vars: response.data
        }),
