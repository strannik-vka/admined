...
class Admined extends React.Component {
  ...
  onFilterChange = (value, name, callback) => {
    ...
    this.setState({ ...this.pageDefault(),
      ...{
        page: pageData
      }
    }, () => {
  ...
  changePage = (page, reset) => {
    ...
    pageDefault.page = { ...pageDefault.page,
      ...page
    };
        ...
        data.form.forEach((item, i) => {
          ...
          if (typeof item.fields === 'object' && item.fields != null) {
            item.fields.forEach((element, index) => {
              data.form[i].fields[index].type = data.form[i].fields[index].type ? data.form[i].fields[index].type : 'string';
            });
          }
        });
      }
